...
import AutoClaimAbi from './abis/AutoClaim'
import { autoClaimChainIdAtom } from './atoms'
import { AUTO_CLAIM_ADDRESSES } from './config'

export const AutoClaimList = () => {
  const [chainId] = useAtom(autoClaimChainIdAtom)
  const tokenFaucetAddresses = AUTO_CLAIM_ADDRESSES[chainId]
...
const TokenFaucetItem: React.FC<{ prizePool: V3PrizePool; tokenFaucetAddress: string }> = (
  ...
  const submitTransaction = async () => {
    const callTransaction = () => {
      const faucetContract = new ethers.Contract(tokenFaucetAddress, AutoClaimAbi, signer)
